...
백엔드 팀은 외부 항공편 API 연동, 가격 확인·이메일 발송, 인증 및 J플랜 답변 생성 등을 담당했습니다. 프론트엔드에서는 해당 API를 연결하고 응답을 사용자 화면에 반영했습니다.
...
백엔드의 구간별 응답을 화면에서 사용하는 공통 구조로 변환하는 함수를 분리했습니다. 가는 편·오는 편의 시간과 전체 가격을 매핑하고, 검색·알림 데이터를 공통 상세 화면에서 사용할 수 있도록 정리했습니다.

현재 결과 목록은 같은 가는 편을 기준으로 항목을 묶고, 그룹 내 최저가 항목을 대표로 표시합니다. 이를 통해 API 응답을 그대로 나열하는 것과 사용자가 비교하기 좋은 목록을 구성하는 것은 다른 문제라는 점을 배웠습니다.
...
- 같은 가는 편을 기준으로 검색 결과를 묶고 최저가 항목을 대표로 표시
